...
import pandas as pd
import qrcode
from fpdf2 import FPDF # CORREÇÃO CRÍTICA: Use fpdf2, que está no requirements.txt
...
def gerar_pdf_com_qr(dados_linha, base_url):
    ...
    # Usando o nome da primeira coluna do CSV como ID (no seu caso, 'Código')
...
def index():
    ...
    # O template 'index.html' também está em 'templates/index.html', o Flask resolve isso
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            # 3. Validação dos Dados - Adaptado para usar a primeira coluna, não 'ID_UNICO' fixo
            # O CSV de teste possui a primeira coluna como 'Código'
            first_column_name = df.columns[0]
            if df.empty or first_column_name not in ['ID_UNICO', 'Código']:
                 return "O arquivo está vazio ou a primeira coluna ('ID_UNICO' ou 'Código') não foi identificada.", 400
            ...
            # Mensagem de erro mais amigável para o front-end
